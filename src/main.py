...
class BLE_Sensor:
    ...
    def __init__(self, name):
        ...
        self.sensor_values = {"raw": None,"timestamp":None,  "name": None, "unit": None, "warmup1": None, "warmup2": None, "activated": None}
...
def is_sensor_error(value):
    if(value[1]==0xff and value[2]==0xff and value[3]==0xff and value[4]==0xff and value[5]==0xff and value[6]==0xff):
        return True
    else:
        return False
...
class BLEDevice(Device):
    ...
    def characteristic_value_updated(self, characteristic, value):
        ...
        sensor_addr = value[0]
        sensor = sensor1 if sensor_addr == 0x1 else sensor2 if sensor_addr == 0x2 else None
        if (sensor == None):
        ...
        float_len = sensor.sensor_values["raw"][2]
        print([hex(v) for v in sensor.sensor_values["raw"]])
        value.hex()[6:6+2*float_len]
            
        
        decoded = data_to_float(value)
        
        line = f"""{now} : {decoded} : {value}"""
