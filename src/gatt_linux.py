...
class DeviceManager:
    ...
    def run(self):
        ...
        self._interface_added_signal = self._bus.add_signal_receiver(
            self._interfaces_added, dbus_interface="org.freedesktop.DBus.ObjectManager", signal_name="InterfacesAdded"
        )
...
class Device:
    ...
    def _connect(self):
        ...
        try:
            ...
            if e.get_dbus_name() == "org.freedesktop.DBus.Error.UnknownObject":
                ...
                and (e.get_dbus_name() == "org.bluez.Error.Failed")
                and (e.get_dbus_message() == "Software caused connection abort")
            ):
                self._connect()
    ...
    def services_resolved(self):
        ...
        managed_services = [
            service for service in self._object_manager.GetManagedObjects().items() if services_regex.match(service[0])
        ]
        self.services = [
            Service(device=self, path=service[0], uuid=service[1]["org.bluez.GattService1"]["UUID"])
...
class Service:
    ...
    def characteristics_resolved(self):
        ...
        managed_characteristics = [
            char for char in self._object_manager.GetManagedObjects().items() if characteristics_regex.match(char[0])
        ]
        self.characteristics = [
            Characteristic(service=self, path=c[0], uuid=c[1]["org.bluez.GattCharacteristic1"]["UUID"])
            for c in managed_characteristics
        ]
    ...
    def read_value(self, offset=0):
        ...
        try:
            ...
            val = self._object.ReadValue(
                {"offset": dbus.UInt16(offset, variant_level=1)}, dbus_interface="org.bluez.GattDescriptor1"
            )
            return val
        except dbus.exceptions.DBusException as e:
...
class Characteristic:
    ...
    def read_value(self, offset=0):
        ...
        try:
            return self._object.ReadValue(
                {"offset": dbus.UInt16(offset, variant_level=1)}, dbus_interface="org.bluez.GattCharacteristic1"
            )
        except dbus.exceptions.DBusException as e:
            error = _error_from_dbus_error(e)
    ...
    def _enable_notifications_failed(self, dbus_error):
        ...
        if (dbus_error.get_dbus_name() == "org.bluez.Error.Failed") and (
            (dbus_error.get_dbus_message() == "Already notifying")
            or (dbus_error.get_dbus_message() == "No notify session started")
        ):
